/*
	Lightbox - required: mootools.js v1.2 core + more: Assets
	v1.3.1
		- feature: hide Lightbox links hover default firefox tooltips
		- feature: hide select and textarea tags while playing
		
		- fix: you can use Lightbox with image links that contains url variables (ex. image.jpg?id=77&lang=ita)
...
var Lightbox = new Class({
	...
	options:{//set all the options here
		...
		initialHeight:150,
		resizeDuration:100,
		resizeTransition:'cubic:out',/*function (ex. Transitions.Sine.easeIn) or string (ex. 'quint:out')*/
	...
	showThisImage:function(image,description){
 	...
 	showImage:function(image){
 		...
 		this.image.setStyles({'opacity':0, 'width':0, 'height':0});
		...
		if(imageSize.width > (windowSize.x - 80)) {
			imageSize.height = ((windowSize.x - 80) / imageSize.width) * imageSize.height;
			imageSize.width = (windowSize.x - 80);
		}
		
		if(imageSize.height > (windowSize.y - 130)) {
			imageSize.width = ((windowSize.y - 130) / imageSize.height) * imageSize.width;
			imageSize.height = (windowSize.y - 130);
		}
 		...
 		//if(imageSize.width != centerSize.width){ 
 			...
 			targetSize.marginLeft = -(imageSize.width/2).round() - 7;
			if(targetSize.marginLeft * -1 > Window.getSize().x / 2) targetSize.marginLeft = -Window.getSize().x / 2;
 		//};
		...
		image.setStyles({'width':imageSize.width, 'height':imageSize.height});
	...
	prepareEvents:function(){
		...
		this.galleries.each(function(gallery){
			$$(gallery).addEvent('click',function(e){
				...
				else{ this.openLightbox(gallery, gallery.indexOf(button)); }
		...
		//close
		$$(this.overlay,this.image,this.close).addEvent('click',function(){ this.closeLightbox(); }.bindWithEvent(this));
	...
	autoPlay:function(obj){//obj: gallery, index, delay (in seconds)
		...
		var g = (obj && obj.gallery && ($type(obj.gallery) == 'array')) ? obj.gallery : Lightbox.galleries[0];
	...
	prepareGalleries:function(){
		...
		$$('a').each(function(a){
			...
			if(a.rel && a.rel.test(/^lightbox/i) && a.href.split('?')[0].test(/\.(gif|jpg|png)$/i)){
				if(a.rel.length>8 && !families.contains(a.rel)){ families.push(a.rel); };
				Lightbox_a.push(a);
			}
		},this);

		//console.log(Lightbox_a)
